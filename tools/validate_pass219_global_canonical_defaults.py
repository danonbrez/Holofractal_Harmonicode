...
def main():
 ...
 if c["current_cumulative_binding_census"]["binding_count"]!=len(expected) or len(expected)!=35: raise SystemExit("CONTRACT_BINDING_COUNT_DRIFT")
 ...
 if ceiling!=218 or floor!=186: raise SystemExit("CURRENT_WIRED_BOUNDARY_DRIFT")
 for p in range(floor,ceiling+1):
  if nums.count(p)!=(3 if p==200 else 1): raise SystemExit(f"NUMERIC_COVERAGE_DRIFT:{p}")
 if eh.index('hhs_pass219_inherited_pass186_1_40.h')>eh.index('hhs_pass219_global_canonical_defaults_1_0.h'): raise SystemExit("POLICY_HEADER_NOT_AFTER_CURRENT_INHERITED_TAIL")
 if ec.index('hhs_pass219_inherited_pass186_1_40.inc')>ec.index('hhs_pass219_global_canonical_defaults_1_0.inc'): raise SystemExit("POLICY_SOURCE_NOT_AFTER_CURRENT_INHERITED_TAIL")
 ...
 debtp=c.get("validated_compression_debt_policy",{})
 if debtp.get("contract_id")!="HHS_PASS219_COMPRESSION_DEBT_CLOSURE_3_25_1_0" or not debtp.get("mandatory"): raise SystemExit("GLOBAL_COMPRESSION_DEBT_POLICY_MISSING")
 if debtp.get("conserved_quantity")!="COMPRESSION_DEBT" or debtp.get("elapsed_time_is_debt") is not False or not debtp.get("physical_time_monotonic"): raise SystemExit("GLOBAL_COMPRESSION_DEBT_QUANTITY_DRIFT")
 if debtp.get("reciprocal_normalization")!={"debt":{"numerator":3,"denominator":25},"execution_capacity":{"numerator":25,"denominator":3}}: raise SystemExit("GLOBAL_COMPRESSION_DEBT_RECIPROCAL_DRIFT")
 nb=debtp.get("native_boundary",{})
 if nb.get("bits")!=5184 or nb.get("bytes")!=648 or nb.get("vm81_cells")!=81 or nb.get("x86_word_bits")!=64 or nb.get("hash72_lanes")!=3 or nb.get("hash216_occurrences")!=216 or not nb.get("complete_sha256_positional_index_required"): raise SystemExit("GLOBAL_COMPRESSION_DEBT_NATIVE_BOUNDARY_DRIFT")
 if debtp.get("immediate_active_cells_max")!=7 or debtp.get("reference_81_over_7_x1000")!=11571: raise SystemExit("GLOBAL_COMPRESSION_DEBT_ACTIVE_SURFACE_DRIFT")
 if not debtp.get("reciprocal_transfer_required") or not debtp.get("no_anonymous_debt") or not debtp.get("timing_noncanonical") or not debtp.get("singleton_vm81_authority_preserved") or not debtp.get("hash72_hash216_authority_preserved"): raise SystemExit("GLOBAL_COMPRESSION_DEBT_AUTHORITY_DRIFT")
 for token in ["hhs_pass219_compression_debt_closure_3_25_1_0.h","hhs_exact_pass219_global_canonical_defaults_validate_compression_debt_policy"]:
  if token not in eh+read(ROOT/"hhs_runtime/include/hhs_pass219_global_canonical_defaults_1_0.h"): raise SystemExit(f"GLOBAL_COMPRESSION_DEBT_HEADER_WIRING_MISSING:{token}")
 if "hhs_pass219_compression_debt_closure_3_25_1_0.inc" not in ec: raise SystemExit("GLOBAL_COMPRESSION_DEBT_SOURCE_WIRING_MISSING")
 if "hhs_exact_pass219_compression_debt_policy_validate()" not in pi: raise SystemExit("GLOBAL_COMPRESSION_DEBT_GLOBAL_DEFAULT_VALIDATOR_MISSING")
