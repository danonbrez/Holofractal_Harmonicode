...
export async function ingest() {
  ...
  try {
    state.ingress = await requestJson('/api/runtime/multimodal-ingress/ingest', { method: 'POST', body: JSON.stringify(sourcePayload()) });
    ...
    return await loadSnapshot(state.ingress.source?.source_hash);
  } catch (error) { setStage('ingress', 'failed'); log(`Ingress rejected: ${error.message}`); throw error; }
}
export async function loadSnapshot(sourceHash = state.ingress?.source?.source_hash) {
  if (!sourceHash) throw new Error('No ingested source');
  setStage('snapshot', 'running');
  state.snapshot = await requestJson(`/api/runtime/multimodal-ingress/snapshots/${encodeURIComponent(sourceHash)}`);
...
async function workspace(operation, payload = {}) {
  return requestJson('/api/runtime/workspace/command', { method: 'POST', body: JSON.stringify({ operation, payload: { project_id: await ensureProject(), ...payload } }) });
}
export async function interpret() {
  setStage('interpret', 'running');
  const result = await workspace('interpret.execute', { source_object_id: `object:${activeFile().path}`, expression: inferExactExpression() });
...
export async function compile() {
  ...
  const result = await workspace('compile.execute', { source_object_id: `object:${activeFile().path}`, source_text: $('#ide-source-editor').value, target: 'HHS_IR' });
...
export async function run() {
  ...
  const executed = await workspace('emulator.run', { session_id: sessionId, steps: Math.min(32, Math.max(1, Number($('#ide-run-steps').value || 8))) });
  const snapshot = await workspace('emulator.snapshot', { session_id: sessionId });
...
export async function runLifecycle() {
  ...
  try {
    state.lifecycle = await requestJson('/api/runtime/development/lifecycle', {
      ...
      body: JSON.stringify({ ...sourcePayload(), project_id: await ensureProject(), project_name: 'HHS Visual IDE Project', expression: inferExactExpression(), interpretation_scope: 'SOURCE_EXACT_NUMERIC_PROBE', target: 'HHS_IR', steps: Math.min(32, Math.max(1, Number($('#ide-run-steps').value || 8))) }),
    });
...
export async function replay() {
  const result = await requestJson('/api/runtime/development/replay', { method: 'POST', body: '{}' });
  setText('#ide-receipt-output', JSON.stringify(result, null, 2)); log('Deterministic replay returned.', result);
}
