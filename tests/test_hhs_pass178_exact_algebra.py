...
def test_exact_rational_decimal_pair_and_complex_closure():
    assert ExactRational.coerce("0.125") == ExactRational(1, 8)
    assert ExactRational.coerce([5, 64]) == ExactRational(5, 64)
    assert ExactRational.coerce((6, 8)) == ExactRational(3, 4)
