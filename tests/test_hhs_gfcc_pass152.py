...
from hhs_gfcc.core import build_dependency_graph, canonical_spec, evaluate_dependency_graph, replay_workload, run_representative_workload, vm81_index, vm81_inverse
from hhs_gfcc.manifest import validate_inherited_pass152
from hhs_gfcc.schema import load_canonical_spec
...
def test_gfcc_canonical_spec_is_reached_from_file():
    assert load_canonical_spec(ROOT) == canonical_spec()
...
def test_gfcc_codegen_is_byte_deterministic(tmp_path: Path):
    ...
    for first in sorted((first_root / "generated").rglob("*")):
        if first.is_file():
            assert first.read_bytes() == (second_root / first.relative_to(first_root)).read_bytes()
...
def test_gfcc_native_reachability_compiles_and_executes():
    ...
    if not test_binary.is_file() or not cli_binary.is_file():
        ...
        compile_native(ROOT)
    required = [SUBSYSTEM / "dist" / "libhhs_gfcc.a", SUBSYSTEM / "dist" / "libhhs_gfcc.so", test_binary, cli_binary]
...
def test_gfcc_shader_reachability_compiles_and_reflects_real_spirv():
    if not all(shutil.which(tool) for tool in ("glslangValidator", "spirv-val", "spirv-dis")):
        pytest.skip("shader toolchain unavailable")
    workload = run_representative_workload()
    generate_shaders(workload, SUBSYSTEM)
    manifest = compile_shaders(ROOT)
    assert manifest["reflection_validated"] is True
    assert manifest["uniform_layout"]["size_bytes"] == 48
    assert len(manifest["uniform_layout"]["members"]) == 12
...
def test_gfcc_cli_machine_readable_reachability():
    command = [sys.executable, "-m", "hhs_gfcc.cli", "build-parameters", "--repo", str(ROOT), "--output", "json"]
    ...
    assert len(payload["result"]["source_spec_digest"]) == 64
    assert len(payload["result"]["canonical_result_digest"]) == 64
...
def test_gfcc_negative_matrix():
    ...
    assert report["positive_passed"] == 23
    assert report["negative_passed"] == 25
...
def test_gfcc_replay_byte_identical_canonical_result():
    ...
    assert first == second
    assert replay_workload(first)["match"] is True
...
def test_gfcc_inherited_pass152_evidence_is_concrete():
    evidence = validate_inherited_pass152(ROOT)
...
def test_gfcc_required_python_modules_and_native_api_are_concrete():
    required_modules = {
        "__init__.py", "schema.py", "exact.py", "symbols.py", "dependencies.py", "delta369.py",
        "nonary.py", "vm81.py", "hash72.py", "hash216.py", "geometry.py", "shader.py",
        "collision.py", "codegen_c.py", "codegen_shader.py", "compiler.py", "validator.py",
        "receipts.py", "replay.py", "manifest.py", "package.py", "cli.py",
    }
    ...
    for symbol in (
        ...
        "hhs_gfcc_index_hash216", "hhs_gfcc_build_transform", "hhs_gfcc_build_collision_constraint",
        "hhs_gfcc_enforce_collision", "hhs_gfcc_step", "hhs_gfcc_validate", "hhs_gfcc_replay",
    ):
        assert f"{symbol}(" in native_source
    assert not any(token in native_source for token in ("TODO", "NOT_IMPLEMENTED", "return HHS_GFCC_OK; /* stub"))
