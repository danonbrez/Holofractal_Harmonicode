...
from native_projects.hhs_vm81_native_development.hhs_hash216_vector_resolver_manifest_v1 import (
    build_manifest as build_vector_resolver_manifest,
)

SCHEMA = "HHS_VM81_NATIVE_DEVELOPMENT_LEVEL_0_1_V3"
...
CONTRACT_EXTENSION_PATHS = (
    ...
    "native_projects/hhs_vm81_native_development/hhs_hash216_vector_resolver_manifest_v1.py",
)
...
LEVEL_0_EXPLICIT_GAPS = (
    ...
    "vector_write_append_resize_release_operations",
            ...
            stable(
                {
                    ...
                    "categories": [
                        "HASH216_MEMORY_IDENTITY" if "216" in symbol
                        else "HASH72_COMMITMENT"
                    ],
                    "callable_from_public_abi": True,
                    "binding_status": (
                        "DIRECT_ABI_BOUND" if binding
                        else "DIRECT_ABI_UNBOUND_DEFECT"
                    ),
                    "native_opcode": binding.get("native_opcode") if binding else None,
                    "declared_output_schema": (
                        binding.get("output_schema") if binding else None
                    ),
                    "effective_output_schema": (
                        binding.get("output_schema") if binding else None
                    ),
                    "binding_root_hash72": (
                        binding.get("binding_root_hash72") if binding else None
                    ),
                    "numeric_authority": (
                        "EXACT_OR_STRUCTURAL_CANDIDATE_REQUIRES_EXECUTION_EVIDENCE"
                    ),
                    "role_status": (
                        "DISCOVERED_AND_CONTRACT_BOUND_REQUIRES_COMPLETE_DOSSIER"
                    ),
                    "best_use_status": "PARTIAL_FROM_DIRECT_NATIVE_SMOKE",
                    "limits_status": "PARTIAL_FROM_ABI_AND_NEGATIVE_WRAPPER_ONLY",
                    "composition_status": (
                        "RECIPROCAL_AND_EQUALITY_ROUNDTRIP_EVIDENCED"
                    ),
                }
            )
        )
...
def build_architecture_surface(repo: Path) -> dict[str, Any]:
    ...
    vector_resolver = build_vector_resolver_manifest(repo)

    exposure_by_symbol = {
        entry["native_symbol"]: entry for entry in exposure["entries"]
    }
    registry_by_symbol = {
        entry["abi_symbol"]: entry for entry in pass079["entries"]
    }
            ...
            stable(
                {
                    ...
                    "native_opcode": (
                        registry_entry.get("native_opcode")
                        if registry_entry else None
                    ),
                    "declared_output_schema": (
                        registry_entry.get("output_schema")
                        if registry_entry else None
                    ),
                    "effective_output_schema": _effective_output_schema(
                        symbol, registry_entry
                    ),
        ...
        stable(
            {
                ...
                "prior_registry_claim": (
                    compare_entry.get("output_schema") if compare_entry else None
                ),
                "effective_schema": {
                    "type": "integer",
                    "minimum": 0,
                ...
                "resolution": (
                    "NEW_CONTRACT_LAYER_PRESERVES_C_ABI_AND_OVERRIDES_ONLY_"
                    "THE_DERIVED_SCHEMA_CLAIM"
                ),
            }
        )
    ]
    ...
    direct_entries = [
        entry for entry in entries if entry["callable_from_public_abi"]
    ]
    descriptor_entries = [
        entry for entry in entries if not entry["callable_from_public_abi"]
    ]
    float_entries = [
        entry for entry in entries
        if entry["numeric_authority"] == "NON_AUTHORITATIVE_FLOAT_CONTROL_ONLY"
    ]
    ...
    result = stable(
        {
            ...
            "implementation_slice": (
                "LEVEL_0_1_TYPED_ABI_COMPLETE_HASH_BINDING_AND_"
                "HASH216_IMMUTABLE_VECTOR_RESOLVER"
            ),
            "frozen_sources": _source_records(repo, FROZEN_SOURCE_PATHS),
            "contract_extension_sources": _source_records(
                repo, CONTRACT_EXTENSION_PATHS
            ),
            "vm81_cell_count": GRID_SIZE,
            "hash216_position_count": HASH216_POSITION_COUNT,
            "capability_counts": {
                "catalogued_native_functions": len(entries),
                "legacy_public_c_abi_callable": sum(
                    1 for entry in legacy_entries
                    if entry["callable_from_public_abi"]
                ),
                ...
                "pass079_registered_native_opcodes": (
                    pass079["registered_native_opcodes"]
                ),
                "hash_native_registered_bindings": (
                    hash_bindings["registered_bindings"]
                ),
                "non_authoritative_float_control_symbols": len(float_entries),
                "contract_scoped_typed_execution_abi_functions": 1,
                "contract_scoped_vector_resolver_abi_functions": 5,
            },
            "entries": entries,
            "hash_native_binding_registry_root_hash72": (
                hash_bindings["registry_root_hash72"]
            ),
            "typed_execution_result_abi": {
                "header": (
                    "native_projects/hhs_vm81_native_development/c/"
                    "hhs_vm81_native_dev_abi.h"
                ),
                "implementation": (
                    "native_projects/hhs_vm81_native_development/c/"
                    "hhs_vm81_native_dev_abi.c"
                ),
                "abi_version": 1,
                ...
                "authority_admission_marker": (
                    "PREVALIDATED_MARKER_NOT_SINGLE_USE_MUTATION_CAPABILITY"
                ),
            },
            "hash216_vector_resolver": {
                "manifest_root_hash72": (
                    vector_resolver["resolver_manifest_root_hash72"]
                ),
                "abi_version": vector_resolver["abi_version"],
                "address_tuple": vector_resolver["address_tuple"],
                "operations": vector_resolver["operations"],
                "statuses": vector_resolver["statuses"],
                "limits": vector_resolver["limits"],
                "immutable_snapshot": True,
                "runtime_mutation_surface_exposed": False,
            },
            "contract_errata": contract_errata,
            "initial_level1_execution_symbols": list(
                LEVEL_1_INITIAL_EXECUTION_SYMBOLS
            ),
            "explicit_missing_capabilities": list(LEVEL_0_EXPLICIT_GAPS),
            "closure": {
                "all_native_functions_classified": exposure["complete_catalogue"],
                "all_legacy_direct_abi_capabilities_bound": (
                    sum(
                        1 for entry in legacy_entries
                        if entry["callable_from_public_abi"]
                    ) == pass079["registered_native_opcodes"]
                ),
                "all_hash72_hash216_direct_abi_capabilities_bound": (
                    hash_bindings["all_expected_bindings_present"]
                ),
                "all_current_linked_direct_abi_capabilities_bound": all(
                    entry["binding_status"] == "DIRECT_ABI_BOUND"
                ...
                "hash216_logical_address_resolver_implemented": (
                    vector_resolver["closure"]
                    ["canonical_hash216_address_implemented"]
                ),
                "bounded_immutable_vector_descriptor_implemented": (
                    vector_resolver["closure"]
                    ["bounded_immutable_vector_descriptor_implemented"]
                ),
                "bounded_vresolve_implemented": (
                    vector_resolver["closure"]["bounded_vresolve_implemented"]
                ),
                "bounded_vread_implemented": (
                    vector_resolver["closure"]["bounded_vread_implemented"]
                ),
                "vector_mutation_capability_implemented": False,
                "hash72_compare_schema_erratum_applied": True,
                "float_surfaces_excluded_from_authoritative_numeric_execution": all(
                    entry["numeric_authority"]
    ...
    result["architecture_surface_root_hash72"] = product_root(
        "hhs_vm81_native_development_level_0_1_v3", result
    )
...
def write_artifacts(repo: Path) -> dict[str, Any]:
    ...
    (out_dir / "HASH216_IMMUTABLE_VECTOR_RESOLVER_MANIFEST.json").write_text(
        json.dumps(vector_resolver, indent=2) + "\n", encoding="utf-8"
    )
    return result
...
def main() -> int:
    ...
    result = (
        write_artifacts(args.repo)
        if args.write else build_architecture_surface(args.repo)
    )
