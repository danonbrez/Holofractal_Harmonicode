...
SCHEMA = "HHS_VM81_NATIVE_DEVELOPMENT_LEVEL_0_1_V1"
...
LEVEL_0_EXPLICIT_GAPS = (
    ...
    "invalid_opcode_dispatch_status",
    "typed_invalid_operand_status",
    "authoritative_vector_store_operations",
    "hash216_functions_absent_from_pass079_native_opcode_registry",
...
def _frozen_sources(repo: Path) -> list[dict[str, Any]]:
...
def _hash_abi_entries(repo: Path) -> list[dict[str, Any]]:
            ...
            stable(
                {
                    ...
                    "binding_status": "DIRECT_ABI_PRESENT_OUTSIDE_PASS079_REGISTRY",
                    "native_opcode": None,
                    "declared_output_schema": None,
                    "effective_output_schema": {"type": "integer", "enum": [0, 1]} if symbol.endswith("_equal") else None,
...
def build_architecture_surface(repo: Path) -> dict[str, Any]:
    ...
    hash_abi_entries = _hash_abi_entries(repo)
    ...
    result = stable(
        {
            ...
            "implementation_slice": "LEVEL_0_ARCHITECTURE_DISCOVERY_AND_LEVEL_1_EXACT_ABI_SMOKE_FOUNDATION",
            "frozen_sources": _frozen_sources(repo),
            ...
            "implementation_status": "FOUNDATION_IMPLEMENTED_TERMINAL_VERIFICATION_NOT_YET_CLAIMED",
        }
    )
    result["architecture_surface_root_hash72"] = product_root(
        "hhs_vm81_native_development_level_0_1", result
    )
...
def write_artifacts(repo: Path) -> dict[str, Any]:
    ...
    path = out_dir / "VM81_ARCHITECTURE_SURFACE_LEVEL_0_1.json"
