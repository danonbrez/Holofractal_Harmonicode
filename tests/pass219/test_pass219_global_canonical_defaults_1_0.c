...
int main(void){
 ...
 assert(hhs_exact_pass219_global_canonical_defaults_validate_latency_policy()==HHS_EXACT_STATUS_OK);
 assert(HHS_EXACT_PASS219_GLOBAL_LATENCY_DEFAULT_REQUIRED==1U);
 ...
 assert(p.wired_ceiling_pass==218U&&p.wired_floor_pass==176U&&p.registered_binding_count==45U);
 ...
 assert(hhs_exact_pass219_global_canonical_defaults_binding_at(43U,&k)==HHS_EXACT_STATUS_OK&&k.pass_number==177U);
 assert(hhs_exact_pass219_global_canonical_defaults_binding_at(44U,&k)==HHS_EXACT_STATUS_OK&&k.pass_number==176U);
 assert(hhs_exact_pass219_global_canonical_defaults_binding_at(45U,&k)==HHS_EXACT_STATUS_RANGE_ERROR);
 return 0;
}
