    ...
    for boundary in boundaries:
        result = corpus.record(
            CalibrationMeasurement(
                fixture_id=boundary.metric,
                metric=boundary.metric,
                integer_value=boundary.maximum_integer_value,
                unit=boundary.unit,
                expected_classification="PASS",
                observed_classification="PASS",
                evidence=(f"{boundary.metric}.json",),
            )
        )
        assert result["within_boundary"] is True
    assert corpus.to_dict()["coverage_complete"] is True
...
def test_repair_executor_preserves_contracts_and_runs_planned_tests(tmp_path: Path) -> None:
    ...
    test_unit = tmp_path / "test_unit.py"
    test_unit.write_text("def test_unit():\n    assert True\n", encoding="utf-8")
    plan = RepairPlan(
        defect_id="D1",
    ...
    result = RepairExecutor(tmp_path, timeout_seconds=30).execute(
        plan,
        repair_commands=((sys.executable, "-c", "print('repair applied')"),),
