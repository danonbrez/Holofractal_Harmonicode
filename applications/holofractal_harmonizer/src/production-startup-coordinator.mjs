...
const MAX_ASSISTANT_DEFERRAL_MS = 20_000;
...
window.fetch = async function coordinatedFetch(input, init) {
  if (isAssistantRequest(input)) {
    await waitForRegistryPriorityWindow();
  }
  return originalFetch(input, init);
};

window.HHSProductionStartupCoordinator = Object.freeze({
  schema: 'HHS_PASS161_PRODUCTION_STARTUP_COORDINATOR_V1',
