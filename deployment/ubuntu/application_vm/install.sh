...
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_runtime_init >/dev/null 2>&1; then
...
printf '%s\n' "  sudo -u $SERVICE_USER hhs-vm token issue --principal operator --scope runtime.mutate"
 || fail "native runtime export missing: hhs_runtime_init"
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_validate_abi >/dev/null 2>&1; then
  useradd --system --home /var/lib/hhs/application-vm --shell /usr/sbin/nologin "$SERVICE_USER"
fi
...
printf '%s\n' "Issue an operator token locally with:"
printf '%s\n' "  sudo -u $SERVICE_USER hhs-vm token issue --principal operator --scope runtime.mutate"
 || fail "native runtime export missing: hhs_validate_abi"
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_exact_abi_validate >/dev/null 2>&1; then
  useradd --system --home /var/lib/hhs/application-vm --shell /usr/sbin/nologin "$SERVICE_USER"
fi
...
printf '%s\n' "Issue an operator token locally with:"
printf '%s\n' "  sudo -u $SERVICE_USER hhs-vm token issue --principal operator --scope runtime.mutate"
 || fail "native runtime export missing: hhs_exact_abi_validate"
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_hash216_compute >/dev/null 2>&1; then
  useradd --system --home /var/lib/hhs/application-vm --shell /usr/sbin/nologin "$SERVICE_USER"
fi
...
printf '%s\n' "Issue an operator token locally with:"
printf '%s\n' "  sudo -u $SERVICE_USER hhs-vm token issue --principal operator --scope runtime.mutate"
 || fail "native runtime export missing: hhs_hash216_compute"
