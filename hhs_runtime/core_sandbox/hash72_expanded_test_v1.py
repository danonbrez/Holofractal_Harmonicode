...
from hhs_runtime.core_sandbox.state_generator_expanded_v1 import state_generator_expanded


def run_test():
    seen={}
    for S in state_generator_expanded():
        r=enforce_kernel(S)
        if r.get("status")!="LOCKED":
            continue
        H=tuple(encode_state(S))
        if H in seen and seen[H]!=S:
            return {"collision":True,"state1":seen[H],"state2":S}
        seen[H]=S
