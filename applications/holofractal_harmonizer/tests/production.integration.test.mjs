...
test('public startup gives application controls sole ownership before visual hydration', () => {
  ...
  const coordinatorAuthority = coordinator.indexOf('window.HHSProductionStartupCoordinator = Object.freeze');
  const boundedBootLaunch = coordinator.indexOf("void import('./public-boot.mjs')");
  const applicationLaunch = boot.indexOf("const applicationExperience = launch('application-experience'");
  const browserLaunch = boot.indexOf("const browser = applicationExperience.then");
  const integrationLaunch = boot.indexOf("const productionIntegration = applicationExperience.then");
  const visualLaunch = boot.indexOf("const visualIDE = applicationExperience.then");
  assert.match(coordinator, /void import\('\.\/public-boot\.mjs'\)/);
  assert.match(coordinator, /\.then\(\(\{ startPublicBoot \}\) => startPublicBoot\(\)\)/);
  assert.doesNotMatch(coordinator, /^import \{ startPublicBoot \} from '\.\/public-boot\.mjs';?$/m);
  assert.match(coordinator, /window\.HHSProductionStartupCoordinator = Object\.freeze/);
  assert.ok(coordinatorAuthority >= 0);
  assert.ok(boundedBootLaunch > coordinatorAuthority);
  ...
  assert.match(boot, /const browser = applicationExperience\.then\(\(\) => launch\('browser', '\.\/browser\.mjs'\)\)/);
  assert.match(boot, /const productionIntegration = applicationExperience\.then\(/);
  assert.match(boot, /launch\('production-integration', '\.\/production-integration\.mjs'\)/);
  ...
  assert.ok(applicationLaunch >= 0);
  assert.ok(browserLaunch > applicationLaunch);
  assert.ok(integrationLaunch > applicationLaunch);
  assert.ok(visualLaunch > applicationLaunch);
