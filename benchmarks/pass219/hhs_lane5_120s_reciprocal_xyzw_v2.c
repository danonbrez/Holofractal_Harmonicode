...
static uint64_t configured_window_ns(void) {
    const char *s = getenv("HHS_RECIPROCAL_WINDOW_NS");
    char *end = NULL;
...
static HhsResult run_hhs_window(uint64_t window_ns) {
    HhsResult r;
...
static ConventionalResult run_conventional_window(uint64_t window_ns) {
...
static HhsResult run_hhs_count(uint64_t count) {
    HhsResult r;
    struct timespec started;
    uint64_t index;
    memset(&r, 0, sizeof(r));
    ...
    for (index = 0U; index < count; ++index) {
        Query q = query_at(index);
        uint64_t c1 = 0U, c2 = 0U;
        uint64_t endpoint = hhs_endpoint(q, &c1, &c2);
        r.completed_queries++;
        r.represented_transitions += (hhs_u128)q.k;
        r.descriptor_bits += (hhs_u128)bit_length_u64(q.x0) + (hhs_u128)bit_length_u64(q.k);
        r.affine_compositions += (hhs_u128)c1;
...
static ConventionalResult run_conventional_count(uint64_t count) {
    ConventionalResult r;
    struct timespec started;
    uint64_t index;
    memset(&r, 0, sizeof(r));
    ...
    for (index = 0U; index < count; ++index) {
        Query q = query_at(index);
        uint64_t c = 0U;
        uint64_t endpoint = conventional_endpoint(q, &c);
        r.completed_queries++;
        r.represented_transitions += (hhs_u128)q.k;
        r.descriptor_bits += (hhs_u128)bit_length_u64(q.x0) + (hhs_u128)bit_length_u64(q.k);
        r.matrix_multiplications += (hhs_u128)c;
...
static void print_hhs(const char *id, const char *axis, const char *dataset, const char *mode, const HhsResult *r) {
    printf("{\"type\":\"benchmark\",\"id\":\"%s\",\"axis\":\"%s\",\"architecture\":\"hhs\",\"dataset\":\"%s\",\"mode\":\"%s\",\"elapsed_ns\":%" PRIu64 ",\"completed_queries\":%" PRIu64 ",\"represented_transitions\":\"",
           id, axis, dataset, mode, r->elapsed_ns, r->completed_queries);
...
static void print_conventional(const char *id, const char *axis, const char *dataset, const char *mode, const ConventionalResult *r) {
    printf("{\"type\":\"benchmark\",\"id\":\"%s\",\"axis\":\"%s\",\"architecture\":\"conventional_matrix\",\"dataset\":\"%s\",\"mode\":\"%s\",\"elapsed_ns\":%" PRIu64 ",\"completed_queries\":%" PRIu64 ",\"represented_transitions\":\"",
           id, axis, dataset, mode, r->elapsed_ns, r->completed_queries);
    print_u128(r->represented_transitions);
    printf("\",\"descriptor_bits\":\""); print_u128(r->descriptor_bits);
    printf("\",\"matrix_multiplications\":\""); print_u128(r->matrix_multiplications);
...
int main(void) {
    ...
    printf("{\"type\":\"meta\",\"schema\":\"HHS_120S_RECIPROCAL_XYZW_V2\",\"window_ns\":%" PRIu64 ",\"stream_seed\":%" PRIu64 ",\"modulus\":%" PRIu64 ",\"k_min\":1000000,\"k_span\":1000000000,\"active_threads_per_benchmark\":1,\"benchmarks_sequential\":true,\"dataset_x_producer\":\"A\",\"dataset_y_producer\":\"B\"}\n",
           window_ns, STREAM_SEED, MODULUS);

    a = run_hhs_window(window_ns);
    REQUIRE(a.completed_queries > 0U);
    print_hhs("A", "x", "X", "capacity_120s", &a);

    b = run_conventional_window(window_ns);
    REQUIRE(b.completed_queries > 0U);
    print_conventional("B", "y", "Y", "capacity_120s", &b);

    c = run_conventional_count(a.completed_queries);
    ...
    print_conventional("C", "z", "X", "reciprocal_completion", &c);

    d = run_hhs_count(b.completed_queries);
    ...
    print_hhs("D", "w", "Y", "reciprocal_completion", &d);

    printf("{\"type\":\"verification\",\"dataset\":\"X\",\"producer\":\"A\",\"consumer\":\"C\",\"queries\":%" PRIu64 ",\"endpoint_digest\":%" PRIu64 ",\"descriptor_digest\":%" PRIu64 ",\"exact\":true}\n",
           a.completed_queries, a.endpoint_digest, a.descriptor_digest);
    printf("{\"type\":\"verification\",\"dataset\":\"Y\",\"producer\":\"B\",\"consumer\":\"D\",\"queries\":%" PRIu64 ",\"endpoint_digest\":%" PRIu64 ",\"descriptor_digest\":%" PRIu64 ",\"exact\":true}\n",
           b.completed_queries, b.endpoint_digest, b.descriptor_digest);
