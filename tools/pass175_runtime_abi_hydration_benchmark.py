#!/usr/bin/env python3
"""Benchmark the Pass 175 native ABI and execute the first terminal hydration.

Correctness is authoritative; host timing is explicitly non-authoritative.
"""
...
class FakeAuthority:
    """Deterministic isolated singleton VM81 authority."""
...
def timed(function: Callable[[], Any]) -> tuple[int, Any]:
...
def load_kernel(path: Path) -> ctypes.CDLL:
    ...
    library.hhs175_kernel_address_encode.argtypes = [ctypes.c_uint32, ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint32)]
    library.hhs175_kernel_address_encode.restype = ctypes.c_int
    library.hhs175_kernel_address_decode.argtypes = [ctypes.c_uint32, ctypes.POINTER(KernelAddress)]
    library.hhs175_kernel_address_decode.restype = ctypes.c_int
    library.hhs175_kernel_projected_encode.argtypes = [ctypes.c_uint32, ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint32)]
    library.hhs175_kernel_projected_encode.restype = ctypes.c_int
    library.hhs175_kernel_projected_decode.argtypes = [ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint32), ctypes.POINTER(ctypes.c_uint32)]
    library.hhs175_kernel_projected_decode.restype = ctypes.c_int
    library.hhs175_kernel_control_encode.argtypes = [ctypes.POINTER(ctypes.c_uint8), ctypes.POINTER(ctypes.c_uint16)]
    library.hhs175_kernel_control_encode.restype = ctypes.c_int
    library.hhs175_kernel_control_decode.argtypes = [ctypes.c_uint32, ctypes.POINTER(ctypes.c_uint8)]
    ...
    library.hhs175_kernel_prepare_candidates.argtypes = [ctypes.POINTER(CandidateInput), ctypes.c_size_t, ctypes.POINTER(Candidate)]
    ...
    library.hhs175_kernel_commit_candidates.argtypes = [
        ctypes.POINTER(Candidate), ctypes.c_size_t, ctypes.POINTER(KernelState),
        AdmitCallback, ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint64),
    ]
    library.hhs175_kernel_commit_candidates.restype = ctypes.c_int
    return library


def benchmark_samples(iterations: int, operation: Callable[[int], None]) -> list[int]:
    samples: list[int] = []
    for _ in range(5):
        started = time.perf_counter_ns()
...
def native_abi_benchmark(library_path: Path, iterations: int, batch_size: int) -> dict[str, Any]:
    library = load_kernel(library_path)
    checks: dict[str, bool] = {
        "abi_version": library.hhs175_kernel_abi_version() == ABI_VERSION,
    }

    length = ctypes.c_size_t()
    lo_pointer = library.hhs175_kernel_scalar_lo(ctypes.byref(length))
    lo = bytes(lo_pointer[index] for index in range(length.value))
    hi_pointer = library.hhs175_kernel_scalar_hi(ctypes.byref(length))
    ...
    def address_roundtrip(index: int) -> None:
        cell, operation = index % VM81_CELLS, (index * 17) % 64
        expected = cell * 64 + operation
        if library.hhs175_kernel_address_encode(cell, operation, ctypes.byref(state_out)) != 0:
            raise RuntimeError("address encode failed")
        if state_out.value != expected:
            raise RuntimeError("address encode mismatch")
        if library.hhs175_kernel_address_decode(state_out.value, ctypes.byref(address)) != 0:
    ...
    address_samples = benchmark_samples(iterations, address_roundtrip)
    checks["address_roundtrip"] = True

    projected = ctypes.c_uint32()
    decoded_state, decoded_control = ctypes.c_uint32(), ctypes.c_uint32()

    def projected_roundtrip(index: int) -> None:
        state, control = (index * 67) % STATE_COUNT, (index * 29) % CONTROL_COUNT
        expected = state * CONTROL_COUNT + control
        if library.hhs175_kernel_projected_encode(state, control, ctypes.byref(projected)) != 0:
            raise RuntimeError("projected encode failed")
        if projected.value != expected:
            raise RuntimeError("projected address mismatch")
        if library.hhs175_kernel_projected_decode(
    ...
    projected_samples = benchmark_samples(iterations, projected_roundtrip)
    checks["projected_roundtrip"] = True

    trits, encoded = (ctypes.c_uint8 * CONTROL_TRITS)(), ctypes.c_uint16()
    for control in range(CONTROL_COUNT):
        if library.hhs175_kernel_control_decode(control, trits) != 0:
            raise RuntimeError("control decode failed")
        if library.hhs175_kernel_control_encode(trits, ctypes.byref(encoded)) != 0 or encoded.value != control:
    ...
    inputs, candidates = (CandidateInput * effective_batch)(), (Candidate * effective_batch)()
    for index in range(effective_batch):
        library.hhs175_kernel_bitset_clear(ctypes.byref(inputs[index].read_set))
        library.hhs175_kernel_bitset_clear(ctypes.byref(inputs[index].write_set))
        if library.hhs175_kernel_bitset_add(ctypes.byref(inputs[index].write_set), index) != 0:
    ...
    state, receipt = KernelState(), ctypes.c_uint64()
    pipeline_iterations = max(1, iterations // 100)

    def candidate_pipeline(_: int) -> None:
        library.hhs175_kernel_state_reset(ctypes.byref(state))
        receipt.value = 0
        if library.hhs175_kernel_prepare_candidates(inputs, effective_batch, candidates) != 0:
            raise RuntimeError("candidate prepare failed")
        if library.hhs175_kernel_sort_candidates(candidates, effective_batch) != 0:
            raise RuntimeError("candidate sort failed")
        if library.hhs175_kernel_commit_candidates(
            candidates, effective_batch, ctypes.byref(state), admit, None, ctypes.byref(receipt)
        ) != 0:
    ...
    pipeline_samples = benchmark_samples(pipeline_iterations, candidate_pipeline)
    checks["singleton_vm81_candidate_commit"] = True
    if not all(checks.values()):
        raise RuntimeError(f"native ABI checks failed: {checks}")

    def timing_record(samples: list[int], count: int, label: str) -> dict[str, Any]:
        ...
        return {
            ...
            "operations_per_second_median": rate(count, median),
        }
    ...
    return {
        ...
        "timings_nonauthoritative": {
            "address_roundtrip": timing_record(address_samples, iterations, "roundtrips_per_sample"),
            "projected_roundtrip": timing_record(projected_samples, iterations, "roundtrips_per_sample"),
            "candidate_prepare_sort_commit": timing_record(
                pipeline_samples, effective_batch * pipeline_iterations, "candidates_per_sample"
            ) | {"batch_size": effective_batch, "batches_per_sample": pipeline_iterations},
        },
    }


def first_hydration(repository_root: Path) -> dict[str, Any]:
    authority = FakeAuthority()
    with tempfile.TemporaryDirectory(prefix="hhs-p175-hydration-") as temporary:
        root = Path(temporary)
        initialization_ns, base = timed(lambda: Pass175Runtime(
            authority=authority,
            microcode_store=HydratedMicrocodeStore(root / "base_microcode.jsonl"),
        ))
        secure_store = EncryptedHash216Store(
            root / "secure_microcode.sqlite3", key_path=root / "secure_microcode.key"
        )
        terminal = TerminalPass175Runtime(
            base_runtime=base, secure_store=secure_store, repository_root=repository_root
        )

        hydration_ns, hydration = timed(lambda: terminal.cold_hydrate_terminal(seal=True))
        boot_ns, boot = timed(terminal.boot_firmware)
        requests = [
        ...
        execution_ns, execution = timed(lambda: terminal.execute_batch(requests, max_workers=8))
        replay_ns, replay = timed(terminal.replay)
        status = terminal.status()
        base_status = status["base_runtime"]
        secure_verification = secure_store.verify()
        secure_root = secure_store.root_sha256
        secure_store.close()

    checks = {
        "permanent_instruction_count_5184": base_status["permanent_instruction_count"] == STATE_COUNT,
        "permanent_identity_count_5184": base_status["permanent_identity_count"] == STATE_COUNT,
        "controls_per_instruction_243": base_status["controls_per_instruction"] == CONTROL_COUNT,
        "projected_address_count_1259712": base_status["projected_address_count"] == STATE_COUNT * CONTROL_COUNT,
        "supported_corpus_hydrated": hydration["supported_instruction_forms"] == len(SUPPORTED_CORPUS),
        "hash216_store_sealed": hydration["secure_store"]["sealed"] is True,
        "hash216_positional_indexes": secure_verification["positional_index_count"] >= 216,
        "firmware_ready": boot["ready"] is True,
        "firmware_stage_count_18": len(boot["stages"]) == 18,
        "parallel_execution_candidates": execution["parallel_execution_candidates"] is True,
        "no_parallel_state_authority": execution["parallel_state_authority"] is False,
        "singleton_vm81_authority": execution["singleton_vm81_commit_authority"] is True,
        "single_hash72_commit_stream": execution["hash72_commit_streams"] == 1,
        "deterministic_replay": replay["deterministic_replay"] is True,
        "authority_call_count_20": len(authority.calls) == 20,
    }
    if not all(checks.values()):
    ...
    return {
        ...
        "hydration": {
            "supported_instruction_forms": hydration["supported_instruction_forms"],
            "secure_store_root_sha256": secure_root,
            "sealed": hydration["secure_store"]["sealed"],
            "classification": hydration["classification"],
        },
        "firmware": {
            "ready": boot["ready"],
            "stage_count": len(boot["stages"]),
            "classification": boot["classification"],
        },
        "execution": {
            "candidate_count": execution["candidate_count"],
            "wave_count": execution["wave_count"],
            "parallel_execution_candidates": execution["parallel_execution_candidates"],
            "parallel_state_authority": execution["parallel_state_authority"],
            "singleton_vm81_commit_authority": execution["singleton_vm81_commit_authority"],
            "hash72_commit_streams": execution["hash72_commit_streams"],
        },
        "replay": {
            "deterministic_replay": replay["deterministic_replay"],
            "journal_root_sha256": replay["journal_root_sha256"],
        },
        "timings_nonauthoritative": {
            "runtime_initialization_ns": initialization_ns,
            "cold_hydration_and_seal_ns": hydration_ns,
            "firmware_boot_ns": boot_ns,
            "three_candidate_execution_ns": execution_ns,
            "replay_ns": replay_ns,
        },
    }
...
def main() -> int:
    ...
    if not arguments.abi_library.is_file():
        parser.error(f"ABI library not found: {arguments.abi_library}")

    started = time.time_ns()
    native = native_abi_benchmark(arguments.abi_library, arguments.iterations, arguments.batch_size)
    hydration = first_hydration(arguments.repository_root.resolve())
    report = {
