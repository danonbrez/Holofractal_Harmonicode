...
- Reconciliation v1 census:
  - `PROVEN = 429`
  - `SYMBOLIC = 43`
  - `MISSING_PROJECTION = 0`
  - `UNSUPPORTED_DOMAIN = 0`
- Reconciliation v1 manifest SHA-256:
...
The parser does not rewrite the source into `(c^b)^4` or `c^(b^4)`.
...
During validation, Python's Unicode alphanumeric classification initially caused superscript digits to be consumed by the identifier scanner. The repaired v2 lexer explicitly excludes `⁰¹²³⁴⁵⁶⁷⁸⁹` from identifier characters, allowing superscript nodes to retain their own typed source identity. This is a lexical repair, not an algebraic interpretation.
...
- target families: 2
- parser-limit occurrences before: 5
- parser-limit occurrences after: 0
- AST-bound symbolic occurrences: 5
- syntax binding complete: true
- scalar value complete: false
...
Final reconciled candidate counts remain:
...
- migrated resolution families: 2
- migrated occurrences: 5
- parser-limit profile occurrences: 0
- syntax provenance complete for migrated profiles: true
- scalar value complete: false
- canonical admission authority: false
...
- PR merge-test checkout: `9fec10ce15477b11bc8a59ec610f6386eb8cfeac`
- Python: 3.12.14
...
Fail-closed checks also passed:

- raw corpus strict-completion remains intentionally false at the frozen v1 raw-census layer;
- reconciliation v1 scalar-value completion returns the expected failure condition;
- reconciliation v2 scalar-value completion returns the expected failure condition with 43 open symbolic occurrences and 0 parser-limit profile occurrences.
...
**Implement O2 as an additive exact ordered matrix-power projection proof surface, bind it to the existing scalar proof registry/corpus manifests, run dependency-scoped SPI tests, and repair forward without assigning authority outside VM81.**
