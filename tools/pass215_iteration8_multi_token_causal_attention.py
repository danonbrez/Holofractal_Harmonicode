...
from hhs_backend.runtime.hhs_pass215_iteration8_multi_token_causal_attention_v1 import (
    REAL_MODEL_SHA256,
    VALIDATION_SCHEMA,
    build_multi_token_attention_evidence_from_path,
    compare_replay,
    validate_multi_token_attention_evidence,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Pass 215 Iteration 8 exact multi-token causal attention closure")
    ...
    if args.container:
        evidence = build_multi_token_attention_evidence_from_path(
            args.container,
    ...
    if args.validate:
        ...
        validate_multi_token_attention_evidence(evidence)
        execution = evidence["multi_token_coordinate_forward"]
        _write(args.output, {
            "schema": VALIDATION_SCHEMA,
            "valid": True,
            "semantic_exactness": True,
            "sequence_length": evidence["forward_geometry"]["sequence_length"],
            "multi_token_attention_suite_root_hash216": evidence["multi_token_attention_suite_root_hash216"],
