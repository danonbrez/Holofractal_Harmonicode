#!/usr/bin/env python3
"""Deterministic failover proof for Pass 218 Iteration 19."""
...
def h72(label: str) -> str:
    return hash72_digest({"domain": "HHS-P218-I19-VALIDATION"}, {"label": label})
...
def make_action() -> dict:
    return seal_operator_action(
        request_id="i19-validation-request",
        operator_id="prep",
        action=ACTION,
...
def main() -> int:
    ...
    first = authority(harness, "i19-owner-a", "i19-host-a")
    first_record = first.acquire()
    if first_record is None or first_record["fence_epoch"] != 1:
        raise RuntimeError("P218_I19_FIRST_FENCE_REQUIRED")
    ...
    dispatch = execution_a.reserve_dispatch(claim, executor_id="i19-executor", dispatched_epoch_ns=NOW * 1_000_000_000 + 1)

    harness.expire_owner()
    second = authority(harness, "i19-owner-b", "i19-host-b")
    second_record = second.acquire()
    if second_record is None or second_record["fence_epoch"] != 2:
        raise RuntimeError("P218_I19_SECOND_FENCE_REQUIRED")
    ...
    if restored_dispatch is None or restored_dispatch["record_hash72"] != dispatch["record_hash72"]:
        raise RuntimeError("P218_I19_DISPATCH_NOT_RECOVERED")
    ...
    postconditions = Pass218InMemoryPostconditionLedger(second, execution_b, closure_b)
    before = postconditions.status()
    if before["successful_closure_pending_verification_count"] != 1:
        raise RuntimeError("P218_I19_SUCCESS_MUST_BEGIN_PENDING_VERIFICATION")
    ...
    exact_again = postconditions.record_verification(closure=closure, result=result, observation=observation)
    if exact_again["record_hash72"] != verification["record_hash72"]:
        raise RuntimeError("P218_I19_EXACT_REPLAY_NOT_IDEMPOTENT")

    contradictory = seal_postcondition_observation(
        action=ACTION,
        i12_maintenance_record=rotation,
        observation={
            "active_ca_sha256": NEW_CA,
            ...
            "old_writer_released": True,
            "simultaneous_writer_identities_observed": False,
            "post_linearizable_probe_hash72": h72("contradictory-probe"),
        },
        observed_epoch_ns=result["completed_epoch_ns"] + 2,
    )
    try:
        postconditions.record_verification(closure=closure, result=result, observation=contradictory)
    except Pass218PostconditionReplayRejected:
        contradictory_rejected = True
    else:
        raise RuntimeError("P218_I19_CONTRADICTORY_VERIFICATION_NOT_REJECTED")

    after = postconditions.status()
    if after["successful_closure_pending_verification_count"] != 0 or after["distributed_postcondition_verification_count"] != 1:
        raise RuntimeError("P218_I19_VERIFICATION_STATUS_INVALID")

    summary = {
        "schema": "HHS-P218-I19-DISTRIBUTED-POSTCONDITION-VALIDATION-V1",
        ...
        "successful_execution_terminal_before_effect_verification": True,
        "success_pending_before_observation": True,
        "successor_verified_effect_without_redispatch": True,
        "exact_verification_replay_idempotent": True,
        "contradictory_verification_rejected": contradictory_rejected,
    ...
    print("PASS218_I19_DISTRIBUTED_POSTCONDITION_VERIFICATION=1")
