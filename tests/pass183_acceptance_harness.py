...
def main()->int:
    ...
    for adapter,manifest in VALID_CASES:
        ...
        routes.append({"adapter":adapter,"classification":result["classification"],"closure":e["closure_exact"],"outer_residue":e["outer_modulus"].get("residue"),"legacy_precommit_witness":e["hash216"]["logical_identity_sha256"],"hash216_archive_root":result["hash216_archive"]["archive_root_sha256"]})
        checks[f"archive_{adapter}"]=result["hash216_archive"]["created_after_hash72_closure"] is True and result["hash216_archive"]["mutation_authority"] is False and result["authority_receipt"]["payload"]["hash216_input_authority"] is False
    ...
    replay=runtime.replay();checks["deterministic_replay"]=replay["receipt_chain_valid"] is True;checks["hash216_archival_only"]=replay["hash216_archival_only"] is True and replay["hash216_precommit_authority"] is False;checks["genuine_authority"]=not args.isolated and replay["authority_replay"]["singleton_vm81"] is True
