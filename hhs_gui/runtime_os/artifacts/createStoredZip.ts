...
function asBytes(value: StoredZipEntry["data"]): Uint8Array {
...
export function crc32(value: StoredZipEntry["data"]): number {
...
export function createStoredZip(entries: StoredZipEntry[]): Uint8Array {
  ...
  for (const entry of entries) {
    ...
    if (!path || path.includes("\0") || path.split("/").includes("..")) {
      throw new Error(`ZIP_PATH_REJECTED:${path || "<empty>"}`)
    }
    ...
    centralRecords.push(concat([
      ...
      u32(checksum), u32(dataBytes.length), u32(dataBytes.length), u16(nameBytes.length),
