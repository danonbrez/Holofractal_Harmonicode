...
def prove_global_zero_sum_closure(*, center_P: Any = 4) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema": SCHEMA,
        "pass_id": PASS_ID,
        "status": "ADDITIVE_GLOBAL_RELATION_BRIDGE_PROVED",
        "closure_family": {
            "P": inherited["derived"]["P"], "p": inherited["derived"]["p"], "q": inherited["derived"]["q"],
...
def verify_global_zero_sum_closure(proof: dict[str, Any]) -> dict[str, Any]:
    ...
    if root != _hash("hhs_pass219b_global_relation_bridge_v3", body):
    ...
    if _fraction(family["delta"]) != 1 or _fraction(family["p"]) != P - 1 or _fraction(family["q"]) != P + 1:
        raise GlobalZeroSumClosureError("zero-sum family mismatch")
    if _fraction(family["P_squared_minus_pq"]) != 1 or _fraction(family["xy_projection"]) != 1 or _fraction(family["zw_projection"]) != 1:
        raise GlobalZeroSumClosureError("unit projection mismatch")
    if _fraction(family["x_plus_y_plus_z_plus_w"]) != 0 or family["phase_carrier_sum_basis_1_I"] != [0, 0] or _fraction(family["membrane_residue"]) != 1:
        raise GlobalZeroSumClosureError("closure mismatch")
    if body["global_tensor_binding"] != {"symbol": "N", "source_sha256": PARENT_MONOLITHIC_SHA256, "indivisible": True}:
        raise GlobalZeroSumClosureError("N binding mismatch")
    expected_phase = {"symbol": "D", "source": PHASE_QUANTIZATION_OBJECT, "source_sha256": PHASE_QUANTIZATION_SHA256,
        "recursive_relation": "N/D^4=D^4", "structural_projection_registered": True, "scalar_cancellation_allowed": False}
    if body["phase_quantization_binding"] != expected_phase or sha256(PHASE_QUANTIZATION_OBJECT.encode("utf-8")).hexdigest() != PHASE_QUANTIZATION_SHA256:
        raise GlobalZeroSumClosureError("D binding mismatch")
    expected_hydration = {"lo_shu_sudoku_qudit_bound": True, "cell_count81": 81, "lo_shu_group_count41": 41,
        "trit_count3": 3, "vm5184_slot_count": 5184, "hydration_state_count": HYDRATION_STATE_COUNT,
        "phase_origin_count81": 81, "phase_projected_state_count": PHASE_PROJECTED_STATE_COUNT}
    if body["hydration_bridge"] != expected_hydration:
        raise GlobalZeroSumClosureError("hydration bridge mismatch")
    expected_authority = {"i6_projection_id": "PI-UCE-N-D-HYDRATION-I6-v1", "i6_projection_is_additive": True,
        "uqcel_integer_symmetric_reused_as_subprojection": True, "legacy_full_symbolic_v1_preserved_unsupported": True,
        "legacy_full_symbolic_residual_mask": "HHS_UQCEL_RESIDUAL_FULL_SOURCE", "projection_equality_is_not_native_identity": True,
        "canonical_mutation_authority": False, "canonical_hash72_authority": False, "canonical_persistence_authority": False}
    ...
    phase_step = _step(inherited, "FOUR_PHASE_CARRIER_ZERO_SUM")
    if _fraction(inherited["derived"]["p"]) != _fraction(family["p"]) or _fraction(inherited["derived"]["q"]) != _fraction(family["q"]):
        raise GlobalZeroSumClosureError("Pass129 center replay mismatch")
    if _fraction(request["xyzw_sum"]) != 0 or phase_step["output"]["sum"] != [0, 0]:
        raise GlobalZeroSumClosureError("Pass129 closure replay mismatch")
    if body["inherited_pass129"] != {"proof_root_hash72": inherited["proof_root_hash72"], "phase_zero_sum_step_root_hash72": phase_step["step_root_hash72"]}:
...
__all__ = ["SCHEMA", "PASS_ID", "CLOSURE_EXTENSION_SHA256", "PARENT_MONOLITHIC_SHA256", "PHASE_QUANTIZATION_OBJECT",
    "PHASE_QUANTIZATION_SHA256", "HYDRATION_STATE_COUNT", "PHASE_PROJECTED_STATE_COUNT", "GlobalZeroSumClosureError",
    "prove_global_zero_sum_closure", "verify_global_zero_sum_closure"]
