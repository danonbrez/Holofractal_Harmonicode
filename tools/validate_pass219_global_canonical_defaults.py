...
def main():
 ...
 if c["current_cumulative_binding_census"]["binding_count"]!=len(expected) or len(expected)!=37: raise SystemExit("CONTRACT_BINDING_COUNT_DRIFT")
 ...
 if ceiling!=218 or floor!=184: raise SystemExit("CURRENT_WIRED_BOUNDARY_DRIFT")
 for p in range(floor,ceiling+1):
  if nums.count(p)!=(3 if p==200 else 1): raise SystemExit(f"NUMERIC_COVERAGE_DRIFT:{p}")
 if eh.index('hhs_pass219_inherited_pass184_1_42.h')>eh.index('hhs_pass219_global_canonical_defaults_1_0.h'): raise SystemExit("POLICY_HEADER_NOT_AFTER_CURRENT_INHERITED_TAIL")
 if ec.index('hhs_pass219_inherited_pass184_1_42.inc')>ec.index('hhs_pass219_global_canonical_defaults_1_0.inc'): raise SystemExit("POLICY_SOURCE_NOT_AFTER_CURRENT_INHERITED_TAIL")
