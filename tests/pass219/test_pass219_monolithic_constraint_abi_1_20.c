...
int main(int argc, char **argv) {
    ...
    static const uint32_t expected_native_offsets[HHS_EXACT_PASS219_MONOLITHIC_EDGE_COUNT] = {
        11U, 27U, 39U, 53U, 95U, 239U, 265U, 273U, 284U, 329U
    };
    ...
    if (hhs_exact_pass219_monolithic_source(source, sizeof(source), &source_length) !=
            HHS_EXACT_STATUS_OK || source_length != sizeof(source))
        return 3;
    if (hhs_exact_pass219_monolithic_native_source(
            native_source, sizeof(native_source), &native_source_length) !=
            HHS_EXACT_STATUS_OK || native_source_length != sizeof(native_source))
        return 4;

    if (argc == 2 && strcmp(argv[1], "--dump-source") == 0) {
        return fwrite(source, 1U, source_length, stdout) == source_length ? 0 : 5;
    }
    if (argc == 2 && strcmp(argv[1], "--dump-native-source") == 0) {
        return fwrite(native_source, 1U, native_source_length, stdout) == native_source_length ? 0 : 6;
    }

    for (i = 0U; i < HHS_EXACT_PASS219_MONOLITHIC_EDGE_COUNT; ++i) {
        HHSExactPass219MonolithicEdgeV1 edge;
        HHSExactPass219MonolithicEdgeV1 native_edge;
        if (hhs_exact_pass219_monolithic_edge(i, &edge) != HHS_EXACT_STATUS_OK ||
            hhs_exact_pass219_monolithic_native_edge(i, &native_edge) != HHS_EXACT_STATUS_OK)
            return 7;
        if (edge.ordinal != i || edge.byte_offset != expected_offsets[i] ||
            native_edge.ordinal != i || native_edge.byte_offset != expected_native_offsets[i] ||
            edge.paren_depth != expected_paren[i] || native_edge.paren_depth != expected_paren[i] ||
            edge.brace_depth != expected_brace[i] || native_edge.brace_depth != expected_brace[i] ||
            edge.bracket_depth != 0U || native_edge.bracket_depth != 0U ||
            edge.kind != expected_kind[i] || native_edge.kind != expected_kind[i] ||
            edge.token_length != (expected_kind[i] == HHS_EXACT_PASS219_MONOLITHIC_EDGE_CONSTRAINT ? 2U : 1U) ||
            native_edge.token_length != edge.token_length)
            return 8;
    }
    ...
    /* A syntactically complete caller packet is ready for VM81, never self-proven. */
    ...
    if (hhs_exact_pass219_monolithic_verify_proof(&proof, &verification) != HHS_EXACT_STATUS_OK ||
        ...
        verification.proof_packet_complete != 1U ||
        verification.requires_vm81_authority != 1U ||
