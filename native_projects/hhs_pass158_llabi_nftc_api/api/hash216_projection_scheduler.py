...
class Hash216ProjectionScheduler:
    ...
    def __init__(
        ...
        *,
        ...
        self._continuations: dict[str, ChunkRecord] = {}
    ...
    def _make_chunk(
        ...
        *,
        ...
        cached = self._continuations.get(content_hash)
        ...
        if cached is not None:
            ...
            record = ChunkRecord(
                ...
                genome_root=cached.genome_root,
                static=static,
            )
        else:
            positions = tuple(
            ...
            self._continuations[content_hash] = record
    ...
    def observe_runtime_state(
        ...
        *,
        ...
        with self._lock:
            record, changed = self._make_chunk(
                object_id=object_id,
                object_class=object_class,
                value=value,
                static=static,
            ...
            if authoritative:
                if len(self._authoritative) >= self._max_authoritative_events:
    ...
    def _take_events(self, limit: int) -> list[ProjectionEvent]:
        selected: list[ProjectionEvent] = []
        while self._authoritative and len(selected) < limit:
            selected.append(self._authoritative.popleft())
        while self._transient and len(selected) < limit:
            _, event = self._transient.popitem(last=False)
    ...
    def build_projection_package(
        ...
        *,
        ...
        with self._lock:
            ...
            events = self._take_events(budget.max_events_per_package)
            chunk_roots: set[str] = set()
            chunks: list[ChunkRecord] = []
            delta_vectors: list[dict[str, Any]] = []
            authoritative_events = 0
            for event in events:
                record = self._objects[event.object_id]
                if record.genome_root not in chunk_roots and len(chunks) < budget.max_chunks_per_package:
        ...
        with self._lock:
            ...
            for object_id, raw in snapshot.get("objects", {}).items():
                record = ChunkRecord(
                    ...
                    dependency_roots=tuple((str(a), str(b)) for a, b in raw.get("dependency_roots", [])),
                ...
                self._objects[object_id] = record
                self._continuations[record.content_sha256] = record
            self._authoritative = deque(
            ...
            self._latest_budget = FrameBudgetDecision(**snapshot.get("latest_budget", asdict(self._default_budget())))
            return self.status()
