#include "hhs_pass219_inherited_pass213_1_16.hpp"
...
static HHSExactPass213ClosureWitnessV1 witness() {
...
int main() {
    auto input = witness();
    hhs::rna::InheritedPass213CompiledROMAuthority binding(input);
    assert(binding.status() == HHS_EXACT_STATUS_OK);
    assert(binding.wired());
    assert(binding.inherited_governed_mutation_authority());
    assert(binding.record().pass219_new_mutation_authority == 0U);
    assert(binding.record().cxx_mutation_authority == 0U);
    assert(binding.record().vm81_direct_mutation_authority == 0U);
    input = witness();
    input.pass219_cxx_mutation_authority = 1U;
    hhs::rna::InheritedPass213CompiledROMAuthority rejected(input);
    assert(!rejected.wired());
    std::cout << "PASS219_INHERITED_PASS213_1_16_CPP_OK\n";
    return 0;
}
