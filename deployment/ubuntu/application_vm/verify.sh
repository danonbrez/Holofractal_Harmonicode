...
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_runtime_init >/dev/null 2>&1 || { echo "hhs-vm CLI missing" >&2; exit 2; }
...
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_validate_abi >/dev/null 2>&1 || { echo "hhs-vm CLI missing" >&2; exit 2; }
command -v ss >/dev/null 2>&1 || { echo "ss command missing" >&2; exit 2; }
...
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_exact_abi_validate >/dev/null 2>&1 || { echo "hhs-vm CLI missing" >&2; exit 2; }
command -v ss >/dev/null 2>&1 || { echo "ss command missing" >&2; exit 2; }
...
nm -D "$RUNTIME_SO" | grep -Eq ' hhs_hash216_compute >/dev/null 2>&1 || { echo "hhs-vm CLI missing" >&2; exit 2; }
command -v ss >/dev/null 2>&1 || { echo "ss command missing" >&2; exit 2; }
