...
function asBytes(value) {
  ...
  throw new TypeError('SHA-256 input must be a string, ArrayBuffer, or typed byte view');
}
...
function fallbackSha256(input) {
  ...
  let encodedLength = bitLength;
  for (let index = paddedLength - 1; index >= paddedLength - 8; index -= 1) {
    padded[index] = Number(encodedLength & 0xffn);
    encodedLength >>= 8n;
  }
...
export async function sha256Bytes(value, { subtle = globalThis.crypto?.subtle } = {}) {
  const bytes = asBytes(value);
  if (subtle?.digest) {
    try {
      return new Uint8Array(await subtle.digest('SHA-256', bytes));
    } catch {
      // A missing or policy-blocked Web Crypto implementation must not stop
      // the non-authoritative browser projection from booting.
    }
  }
...
export async function sha256Hex(value, options = {}) {
  const digest = await sha256Bytes(value, options);
