...
def _repository_root(requested: str | None = None) -> Path:
    configured = Path(
        os.environ.get("HHS_REPOSITORY_ROOT")
        or Path(__file__).resolve().parents[2]
    ).expanduser().resolve()
    if requested is not None and Path(requested).expanduser().resolve() != configured:
        raise Pass184Error(
            "P184_REJECT_REPOSITORY_ROOT_OVERRIDE",
            "API repository_root must equal the configured HHS repository root",
            details={"requested": requested, "configured": str(configured)},
        )
    return configured
