...
MEMBRANE_PATH = P("hhs_runtime/hhs_pass219_cumulative_pass_membrane_i132_pass194.py")
PASS195_MEMBRANE_PATH = P("hhs_runtime/hhs_pass219_cumulative_pass_membrane_i131_pass195.py")
...
PASS194_I132_BASELINE_COMMIT = "e0e0b33f5508beda2eb63ea09b112f89f244d93c"
PASS195_ACCEPTED_PRIMARY_MERGE = "8bcc0921555ecface13113c8a2620415ddb3fdf1"
PASS195_MEMBRANE_BLOB = "913a7f4624574b07b22c22f05a1b07f6656ecbd2"
...
I132_BASELINE_EVOLVING_BLOBS = {
    VISUAL_SERVER_PATH: "998852398931f2e3af2da57ec455211f938b2661",
    MEMBRANE_PATH: "4ef6bdf72f6afd2b17ffb3500bba71cc9bc05a51",
    PASS195_MEMBRANE_PATH: PASS195_MEMBRANE_BLOB,
}
...
def _pass195_successor_evidence() -> Dict[str, Any]:
    _git("merge-base", "--is-ancestor", PASS195_ACCEPTED_PRIMARY_MERGE, "HEAD")
    historical = _git("rev-parse", f"{PASS194_I132_BASELINE_COMMIT}:{PASS195_MEMBRANE_PATH}")
    if historical != PASS195_MEMBRANE_BLOB:
        raise RuntimeError("PASS194_PASS195_HISTORICAL_MEMBRANE_DRIFT")
    if _git_blob(PASS195_MEMBRANE_PATH) != PASS195_MEMBRANE_BLOB:
        raise RuntimeError("PASS194_PASS195_MEMBRANE_IDENTITY_DRIFT")
    return {
        "pass_number": 195,
        "accepted_primary_merge": PASS195_ACCEPTED_PRIMARY_MERGE,
        "membrane_blob": PASS195_MEMBRANE_BLOB,
...
def pass194_membrane_source_evidence() -> Dict[str, Any]:
    ...
    _prove_historical_blob_set(
        PASS194_I132_BASELINE_COMMIT,
        I132_BASELINE_EVOLVING_BLOBS,
        "PASS194_I132_BASELINE_SOURCE_DRIFT",
    )
    for path, expected in STABLE_SOURCE_BLOBS.items():
        if _git_blob(path) != expected:
            raise RuntimeError(f"PASS194_IMPLEMENTED_SOURCE_DRIFT:{path}")

    _require(
        RUNTIME_PATH,
        "PRAGMA journal_mode=WAL", "PRAGMA synchronous=FULL", 'default_training_allowed\": False',
        "HHS_P194_TRAINING_CONSENT_DENIED", "PersistentEncryptedVectorStore",
        'vector_store_is_source_authority\": False', 'snapshot_is_training_authorization\": False',
        'training_provider_is_vm81_authority\": False', "HHS_P194_VM81_AUTHORITY_RECEIPT_REUSED",
        'deterministic_replay\": True',
    )
    _require(
        API_PATH,
        "/api/runtime/storage-training", "MAX_UPLOAD_BYTES", "Pass194Runtime", "authority_execution",
    )
    _require(
        VISUAL_SERVER_PATH,
        "pass194_storage_training_router", "/api/runtime/storage-training/status",
        "pass194_storage_training_api",
    )
    _require(
        RUNTIME_TEST_PATH,
        "SNAPSHOT_BYTES", "test_default_deny_requires_consent_and_fresh_snapshot",
    ...
    successor = _pass195_successor_evidence()
    return {
        ...
        "i132_baseline_commit": PASS194_I132_BASELINE_COMMIT,
        "stable_source_blobs": {str(path): value for path, value in STABLE_SOURCE_BLOBS.items()},
        "baseline_evolving_blobs": {str(path): value for path, value in I132_BASELINE_EVOLVING_BLOBS.items()},
        "current_evolving_blobs": {str(path): _git_blob(path) for path in I132_BASELINE_EVOLVING_BLOBS},
...
def validate_pass194_contract_and_lineage() -> Dict[str, Any]:
    ...
    return {
        ...
        "i132_baseline_commit": evidence["i132_baseline_commit"],
        "historical_contract_preserved": True,
        "historical_i132_evolving_surface_identities_preserved": True,
...
def validate_pass194_content_and_sql_boundary() -> Dict[str, Any]:
    ...
    return {
        "ok": True, "content_addressed_blob_store": True, "blob_identity_immutable": True,
        "versioned_sql_context_graph": True, "sqlite_wal": True, "sqlite_synchronous_full": True,
...
def validate_pass194_consent_dataset_boundary() -> Dict[str, Any]:
    pass194_membrane_source_evidence()
    return {
        "ok": True, "training_default_deny": True, "sharing_default_deny": True,
        "public_default_deny": True, "training_requires_license": True,
        "dataset_requires_snapshot_consent_closure": True, "consent_drift_requires_new_snapshot": True,
    }
...
def validate_pass194_vector_snapshot_boundary() -> Dict[str, Any]:
    pass194_membrane_source_evidence()
    return {
        "ok": True, "encrypted_hash216_vector_projection": True,
        "vector_frame_uses_inherited_vmrc_snapshot_bytes": True,
        "vector_store_is_source_authority": False, "vector_store_is_consent_authority": False,
        "vector_store_is_vm81_authority": False, "hydration_snapshot_immutable": True,
...
def validate_pass194_training_checkpoint_boundary() -> Dict[str, Any]:
    pass194_membrane_source_evidence()
    return {
        "ok": True, "training_run_kinds_bounded": True, "dataset_lineage_bound": True,
        "checkpoint_artifact_sha256_bound": True, "training_provider_is_vm81_authority": False,
...
def validate_pass194_revocation_replay_boundary() -> Dict[str, Any]:
    pass194_membrane_source_evidence()
    return {
        "ok": True, "file_delete_tombstone": True, "dataset_revocation_propagates": True,
        "training_run_revocation_propagates": True, "orphan_blob_removal": True,
        "receipt_chain_replay": True, "snapshot_identity_replay": True, "dataset_identity_replay": True,
    }
...
def validate_pass194_no_new_authority() -> Dict[str, Any]:
    pass194_membrane_source_evidence()
    return {
        "ok": True, "i132_new_candidate_authority": False,
        "i132_new_canonical_mutation_authority": False, "i132_new_persistence_authority": False,
        "i132_new_hash72_clock": False, "cxx_mutation_authority": False,
        "vm81_mutation_authority": False, "vector_store_is_source_authority": False,
        "vector_store_is_consent_authority": False, "browser_is_authority": False,
        "training_provider_is_vm81_authority": False, "singleton_vm81_authority_remains_inherited": True,
    }
...
def pass194_surface_declaration() -> Dict[str, Any]:
    pass194_membrane_source_evidence()
    return {
        "surface_id": PASS194_SURFACE_ID, "surface_type": "VALIDATOR",
        ...
        "witness_schemas": ["HHSExactPass194StorageTrainingSnapshotAuthorityWitnessV1", "HHSExactPass219InheritedPass194BindingV1"],
        "validators": [PASS194_BIND_SYMBOL, "validate_pass194_contract_and_lineage"],
        "guards": [
            "pass194_historical_contract_identity", "pass194_i132_historical_evolving_identity",
            "pass194_immutable_blob_identity", "pass194_sql_graph_authority",
            "pass194_default_deny_consent", "pass194_dataset_consent_license_closure",
            "pass194_vector_non_authority", "pass194_snapshot_identity",
            "pass194_training_checkpoint_lineage", "pass194_revocation_replay",
            "pass194_pass195_successor_identity", "pass194_no_new_authority",
        ],
        "rejection_codes": [
            "REJECT_PASS194_CONTRACT_DRIFT", "REJECT_PASS194_SOURCE_IDENTITY_DRIFT",
            "REJECT_PASS194_VM81_RECEIPT_BYPASS", "REJECT_PASS194_CONSENT_BYPASS",
            "REJECT_PASS194_DATASET_CLOSURE_BYPASS", "REJECT_PASS194_VECTOR_AUTHORITY_ESCALATION",
            "REJECT_PASS194_SNAPSHOT_DRIFT", "REJECT_PASS194_TRAINING_LINEAGE_DRIFT",
            "REJECT_PASS194_REVOCATION_DRIFT", "REJECT_PASS194_AUTHORITY_ESCALATION",
        ],
...
def pass194_membrane_manifest() -> Dict[str, Any]:
    ...
    return {
        ...
        "pass_number": PASS194_NUMBER, "classification": PASS194_CLASSIFICATION,
        "census_classification": PASS194_CENSUS_CLASSIFICATION,
        "contract_authorization_commit": evidence["contract_authorization_commit"],
        "frozen_predecessor": evidence["frozen_i131"],
        "i132_baseline_commit": evidence["i132_baseline_commit"],
        "surface": pass194_surface_declaration(), "declared_operations": list(REQUIRED_OPERATIONS),
    }
...
def execute_pass194_membrane_preflight() -> Dict[str, Any]:
    ...
    return {
        "schema": "HHS_PASS219_I132_PASS194_PREFLIGHT_V1", "version": VERSION,
        "ok": all(row.get("ok") is True for row in rows), "surface_id": PASS194_SURFACE_ID,
        "operations": rows,
    }
