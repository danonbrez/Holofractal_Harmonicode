...
def test_frozen_rml17_source_and_receipt_nucleus_returns_1001() -> None:
...
def test_accelerated_global_certificate_reuses_frozen_exhaustive_proof() -> None:
...
def test_accelerated_certificate_is_exactly_equal_to_fresh_frozen_rml17_scan() -> None:
...
def test_transport_address_candidate_requires_1001_and_fails_closed() -> None:
    valid = gate_transport_address_candidate(
        rml17.encode_transport_address(3, 63, 71, 80)
    )
    assert valid["status"] == ADMITTED
    assert valid["coordinates"] == [3, 63, 71, 80]
...
def test_route_candidate_returns_1001_only_after_frozen_rml17_equality() -> None:
...
def test_runtime_shape_mutation_fails_closed_without_weakening_rml17(monkeypatch) -> None:
    monkeypatch.setattr(
        rml17,
        "DIRECTION_FLUX",
        {**rml17.DIRECTION_FLUX, "operation_forward": 2},
    )
    candidate = accelerated_address_manifold_certificate()
    assert candidate["status"] == NULL_UNDEFINED
    assert candidate["invariant"] is None
