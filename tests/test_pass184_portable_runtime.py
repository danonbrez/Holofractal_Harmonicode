...
def repository_root(tmp_path: Path) -> Path:
    ...
    (root / "hhs_backend" / "runtime_os_application_server.py").write_text("app = object()\n", encoding="utf-8")
    return root
...
def test_package_build_verify_and_rebuild_are_stable(authority: PortableRuntimeAuthority, repository_root: Path, tmp_path: Path) -> None:
    ...
    assert APP_IMPORT == "hhs_backend.runtime_os_application_server:app"
...
def test_completion_receipt_requires_all_checks(tmp_path: Path) -> None:
    ...
    assert receipt["classification"] == "HHS_PASS_184_I142_LOCAL_ACCEPTANCE_VERIFIED"
    assert receipt["terminal_pass184_completion_claimed"] is False
    assert len(receipt["completion_receipt_hash72"]) == 72
    assert receipt["evidence_set_hash216"]
...
def test_repository_visible_deployment_and_current_runtime_os_contract() -> None:
    ...
    composition = (root / "hhs_backend" / "runtime_os_application_server_full.py").read_text(encoding="utf-8")
    workspace = (root / "hhs_gui" / "runtime_os" / "workspace" / "HHSWorkspaceShell.tsx").read_text(encoding="utf-8")
    panel = (root / "hhs_gui" / "runtime_os" / "workspace" / "Pass184RuntimePackagePanel.tsx").read_text(encoding="utf-8")
    ...
    assert "&" not in service
    assert "app.include_router(pass184_runtime_router)" in composition
    assert "Pass184RuntimePackagePanel" in workspace
    assert "/api/v1/pass184" in panel
    assert "pass184-build" in panel


def test_api_rejects_repository_root_override(monkeypatch: pytest.MonkeyPatch, repository_root: Path, tmp_path: Path) -> None:
    monkeypatch.setenv("HHS_REPOSITORY_ROOT", str(repository_root))
    monkeypatch.setenv("HHS_PASS184_PACKAGE_ROOT", str(tmp_path / "packages"))
    app = FastAPI()
    app.include_router(router)
    client = TestClient(app)
    ...
    assert response.json()["detail"]["status"] == "P184_REJECT_REPOSITORY_ROOT_OVERRIDE"
